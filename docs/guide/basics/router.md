...
`back(stepOrKey?: number|string, target?: RouteTarget, payload?: any, overflowRedirect?: string)`

- 回退溢出：如果回退步数超过历史栈中的记录数，或者目标Key不再历史栈中，即找不到回退目标。  
  此时将直接`relaunch`到第4个参数`overflowRedirect`或者全局设置的`应用首页`，并抛出`ROUTE_BACK_OVERFLOW`错误。  
  这很好理解，就是回退到最后会跳回`首页`（不管历史栈中有没有首页这条历史记录）

- 回退目标为`page`时，并不限于`CurrentWindow下面的PageHistoryStack`，而是将所有PageHistoryStack拼合在一起。也就是说会回退所有历史记录，直到溢出。
...
- `数据前置，路由后置`:在UI跳转前，先把页面所需的数据全部获取准备好。优点是因为UI跳转时数据都已经好了，所以UI上不用设计Loading相关的各种细节，也不用考虑出错页面（如果数据请求出错，路由将停止跳转）；缺点是相对复杂一点，必需根据路由手动编写数据请求的代码。
- `路由前置，数据后置`:UI先跳转，数据根据UI加载自动获取(瀑布式)。优点是简单，不用额外编写数据请求的代码；缺点是UI跳转了，此时数据可能还没请求回来，需要设计Loading的各种细节展示，还得考虑数据请求出处时的展示。
...
1. `store.dispatch({type: 'stage._testRouteChange'})`向Store派发一个`准备跳转`的Action。  
   可以使用effect监听该Action，并决定是不是阻止(**在effect中抛出一个错误**)即可阻止跳转。
...
4. 通知原生路由系统，**也可以强制不通知**，相当于单页应用。
5. `this.dispatch(routeEvent)`自身派发一个路由事件。
6. `store.dispatch({type: 'stage._afterRouteChange'})`向Store派发一个`跳转后`的Action。  
   可以使用effect监听该Action，获取最新的路由信息注入ModuleState中。

### 转场动画

转场动画只能存在于使用`push`和`back`方法跳转路由，并且target为`window`时。

> 因为只有Window历史记录中保存了历史快照(Store和View)，这是一种平衡性能和体验的策略。
...
Elux中的虚拟路由不依赖于原生路由而独立运行，但原生路由可以以`外挂`的模式与虚拟路由建立关联与互动。

它们之间的互动按`发起对象`可分为二种场景：

- 虚拟路由带动原生路由：用户体验较好，可以使用路由拦截、守卫等功能。
- 原生路由带动虚拟路由，可能造成反应滞后或超前的不佳用户体验，且无法拦截。

::: tip 尽量使用Elux虚拟路由

- 除非受到运行平台的限制而无法使用虚拟路由（如:浏览器中用户可以点击后退按钮，操作地址栏等；小程序中用户可以点击手机后退键、点击TabBar跳转等)，否则优先使用Elux虚拟路由进行跳转。

- 虚拟路由的Url可能与原生路由不一致，应用内部流通使用的应当是虚拟路由的Url，而对原生路由Url的转换将在`入口和出口`处进行统一进行。
